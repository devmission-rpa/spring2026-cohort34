  ...
  async function getAllRecords() {
    ...
    try {
      const response = await fetch(
        `https://api.airtable.com/v0/app3knV6H85zkGHHn/Instructors?&view=view`,
        options,
      );
...
window.openDetail = function (data) {
  document.getElementById("modal-name").innerText =
    data[instructorFields.name] || "---";
  document.getElementById("modal-role").innerText =
    data[instructorFields.role] || "---";
  document.getElementById("f-description").innerText =
    data[instructorFields.description] || "---";
  ...
  if (linkedinLink)
    linkedinLink.href = data[instructorFields.linkedIn] || "---";
